...
namespace Orlo.UI.Panels
{
    public class CombatPanelUI : MonoBehaviour
    {
        ...
        // Rendering
        Texture2D _px;
        RacePalette P => TMDTheme.Instance?.Palette ?? RacePalette.Solari;
        Color _green = new Color(0.3f, 0.9f, 0.4f);
        Color _red = new Color(0.9f, 0.3f, 0.3f);

        // Spring for tab underline
        SpringValue _tabSpring;
        float _tabSpringW;
        int _lastTab = -1;
        ...
        void Awake()
        {
            ...
            _px.Apply();
            _tabSpring = new SpringValue(0f, 350f, 0.7f);
        }
        ...
        void Update()
        {
            ...
            _tabSpring.Update(Time.deltaTime);
        }
        ...
        {
            TMDTheme.DrawProgressBar(r, fill, fg);
        }
        bool Button(Rect r, string text, Color textColor, Color bgColor, int size = 12)
        {
            // Use TMDTheme.DrawButton for race-colored buttons
            bool clicked = TMDTheme.DrawButton(r, text);
            return clicked;
        }
        ...
        void OnGUI()
        {
            ...
            TMDTheme.DrawPanel(panelR);

            // Title via TMD
            TMDTheme.DrawTitle(panelR, "COMBAT");

            // Close button
            GUIStyle closeStyle = new GUIStyle(GUI.skin.label)
            {
                fontSize = 14, fontStyle = FontStyle.Bold, alignment = TextAnchor.MiddleCenter,
                normal = { textColor = P.Danger }
            };
            Rect closeR = new Rect(panelR.xMax - 32, panelR.y + 6, 24, 24);
            if (GUI.Button(closeR, "", GUIStyle.none))
            { _visible = false; _pickedAbility = -1; }
            GUI.Label(closeR, "X", closeStyle);

            // Tabs with spring underline
            float tabW = (W - 32) / 4f;
            if (_tab != _lastTab)
            {
                _tabSpring.Target = panelR.x + 16 + _tab * tabW;
                _tabSpringW = tabW;
                _lastTab = _tab;
            }
            if (_tabSpring.Value == 0f && _tabSpring.Target == 0f)
            {
                _tabSpring = SpringPresets.TabSlide(panelR.x + 16 + _tab * tabW, panelR.x + 16 + _tab * tabW);
                _tabSpringW = tabW;
            }
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (!active && hover)
                    DrawRect(tr, new Color(P.Primary.r, P.Primary.g, P.Primary.b, 0.1f));
                Label(tr, _tabNames[i], active ? P.Primary : P.TextDim, 12, TextAnchor.MiddleCenter, active);
            ...
            DrawRect(new Rect(_tabSpring.Value, panelR.y + 62, _tabSpringW, 2), P.Primary);
        ...
        void DrawOverview(Rect area)
        {
            ...
            // Left: Health Pools
            Label(left, "Health Pools", P.Primary, 13, TextAnchor.UpperLeft, true);
            ...
            Label(center, "Armor Resistances", P.Primary, 13, TextAnchor.UpperLeft, true);
            ...
            foreach (var res in _resistances)
            {
                ...
                Label(new Rect(center.x + 72, ry, 36, 16), $"{res.value:F0}", P.Accent, 11, TextAnchor.MiddleCenter);
                Label(new Rect(center.x + 110, ry, 36, 16), $"{res.cap:F0}", Color.grey, 11, TextAnchor.MiddleCenter);
                Color cc = res.conditionPercent > 75 ? _green : (res.conditionPercent > 25 ? P.Accent : _red);
                Label(new Rect(center.x + 148, ry, 48, 16), $"{res.conditionPercent:F0}%", cc, 11, TextAnchor.MiddleCenter);
                ry += 18;
            }

            // Right: Combat Modifiers
            Label(right, "Combat Modifiers", P.Primary, 13, TextAnchor.UpperLeft, true);
            float my = right.y + 22;
            Label(new Rect(right.x, my, right.width, 16), $"Combo: x{_combo}", P.Accent, 12); my += 18;
            ...
            if (_activeEffects.Length > 0)
            {
                ...
                { Label(new Rect(right.x + 8, my, right.width - 8, 14), eff, P.Accent, 11); my += 15; }
            }

            // Bottom: Equipped Weapon
            var wpnR = new Rect(area.x, area.yMax - 56, area.width, 50);
            DrawRect(wpnR, P.PanelBackground);
            DrawBorder(wpnR, P.Border);
            Label(new Rect(wpnR.x + 8, wpnR.y + 4, 160, 16), string.IsNullOrEmpty(_wpnName) ? "No Weapon" : _wpnName, P.Primary, 13, TextAnchor.MiddleLeft, true);
            if (!string.IsNullOrEmpty(_wpnName))
            {
                Label(new Rect(wpnR.x + 180, wpnR.y + 4, 160, 16), $"{_wpnDmgRange} {_wpnDmgType}", P.Accent, 12);
                ...
                DrawBar(new Rect(wpnR.x + 8, wpnR.y + 28, wpnR.width - 16, 10), _wpnCondition,
                    _wpnCondition > 0.75f ? _green : (_wpnCondition > 0.25f ? P.Accent : _red), P.Background);
        ...
        void DrawActionBars(Rect area)
        {
            ...
            if (Button(new Rect(area.x + 62, area.y, 140, 20), $"Bar {_editingBar + 1}", P.Primary, P.PanelBackground, 12))
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                DrawRect(sr, P.PanelBackground);
                if (filled)
                {
                    DrawBorder(sr, P.Primary);
            ...
            // Picked indicator
            if (_pickedAbility >= 0 && _pickedAbility < _abilities.Length)
                Label(new Rect(area.x, barY + slotSize + 4, area.width, 16), $"Placing: {_abilities[_pickedAbility].name} -- click a slot above", P.Accent, 11, TextAnchor.MiddleCenter);

            // Ability palette
            float paletteY = barY + slotSize + 26;
            Label(new Rect(area.x, paletteY, 200, 18), "Abilities", P.Primary, 13, TextAnchor.UpperLeft, true);
            paletteY += 22;
            ...
            foreach (var kv in categories)
            {
                ...
                foreach (int idx in kv.Value)
                {
                    ...
                    DrawRect(ar, picked ? new Color(P.Primary.r, P.Primary.g, P.Primary.b, 0.2f) : P.PanelBackground);
                    DrawBorder(ar, picked ? P.Accent : P.Border);
        ...
        void DrawStances(Rect area)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                DrawRect(col, P.PanelBackground);
                DrawBorder(col, i == _activeStance ? P.Primary : P.Border);

                float cy = col.y + 6;
                string title = $"Stance {i + 1}";
                if (i == _activeStance) title += "  [ACTIVE]";
                Label(new Rect(col.x + 8, cy, colW - 16, 18), title, i == _activeStance ? P.Primary : Color.grey, 13, TextAnchor.UpperLeft, true);
                cy += 24;

                if (i < _stances.Length)
                {
                    var st = _stances[i];
                    Label(new Rect(col.x + 8, cy, colW - 16, 16), st.weaponName, P.Accent, 12, TextAnchor.UpperLeft, true); cy += 20;
                    ...
                    DrawBar(new Rect(col.x + 8, cy, colW - 16, 10), st.condition,
                        st.condition > 0.75f ? _green : (st.condition > 0.25f ? P.Accent : _red), P.Background); cy += 16;
            ...
            DrawRect(new Rect(area.x, compY, area.width, 44), P.PanelBackground);
            DrawBorder(new Rect(area.x, compY, area.width, 44), P.Border);
            Label(new Rect(area.x + 8, compY + 2, 100, 14), "Comparison", P.Primary, 11, TextAnchor.MiddleLeft, true);
            ...
            for (int c = 0; c < 3; c++)
            {
                ...
                if (c < _stances.Length)
                {
                    ...
                    Label(new Rect(cx, compY + 16, colW - 16, 12), $"DPS:{dps:F1} Rng:{s.range:F0}m", P.Accent, 10, TextAnchor.MiddleCenter);
                }
                else
                {
        ...
        void DrawStatistics(Rect area)
        {
            ...
            DrawRect(new Rect(pillX, area.y, pillW, pillH), P.PanelBackground);
            DrawBorder(new Rect(pillX, area.y, pillW, pillH), P.Border);
            var leftPill = new Rect(pillX, area.y, pillW / 2, pillH);
            var rightPill = new Rect(pillX + pillW / 2, area.y, pillW / 2, pillH);
            DrawRect(!_showLifetime ? leftPill : rightPill, new Color(P.Primary.r, P.Primary.g, P.Primary.b, 0.2f));
            Label(leftPill, "This Session", !_showLifetime ? P.Primary : Color.grey, 11, TextAnchor.MiddleCenter);
            Label(rightPill, "Lifetime", _showLifetime ? P.Primary : Color.grey, 11, TextAnchor.MiddleCenter);
            ...
            // Left: Performance
            Label(new Rect(area.x, sy, colW, 16), "Performance", P.Primary, 13, TextAnchor.UpperLeft, true);
            sy += 20;
            ...
            float ry = area.y + 32;
            Label(new Rect(rx, ry, colW - 16, 16), "Damage Dealt by Type", P.Primary, 13, TextAnchor.UpperLeft, true);
            ry += 20;
            DrawDamageBreakdown(rx, ref ry, colW - 16, _dealtBreakdown);
            ry += 10;
            Label(new Rect(rx, ry, colW - 16, 16), "Damage Taken by Type", P.Primary, 13, TextAnchor.UpperLeft, true);
            ry += 20;
            DrawDamageBreakdown(rx, ref ry, colW - 16, _takenBreakdown);

            // Bottom: Recent Kills
            float killY = area.yMax - 90;
            Label(new Rect(area.x, killY, area.width, 16), "Recent Kills", P.Primary, 13, TextAnchor.UpperLeft, true);
            killY += 18;
            DrawRect(new Rect(area.x, killY, area.width, 68), P.PanelBackground);
            DrawBorder(new Rect(area.x, killY, area.width, 68), P.Border);
            float ky = killY + 4;
            ...
            else
            {
                foreach (var kill in _recentKills)
                {
                    ...
                    Label(new Rect(area.x + 260, ky, 80, 13), $"{kill.damage} dmg", P.Accent, 10);
        ...
        void DrawStatRow(float x, ref float y, float w, string label, string value)
        {
            ...
            Label(new Rect(x + w * 0.55f, y, w * 0.45f, 16), value, P.Accent, 11, TextAnchor.UpperRight);
            y += 17;
        }
        ...
        void DrawDamageBreakdown(float x, ref float y, float w, DamageBreakdown[] data)
        {
            ...
            foreach (var d in data)
            {
                ...
                Label(new Rect(x + w - 46, y, 46, 14), $"{d.value:F0}", P.Accent, 10, TextAnchor.UpperRight);
                y += 16;
            }
        }
